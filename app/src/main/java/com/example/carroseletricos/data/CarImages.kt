package com.example.carroseletricos.data

import com.example.carroseletricos.R

class CarImages {
    val carMap = mapOf(
        1 to R.drawable.audietron, 2 to R.drawable.audietrongt1,
        3 to R.drawable.audiq4etron, 4 to R.drawable.bmwi3,
        5 to R.drawable.bmwi4, 6 to R.drawable.bmwix3,
        7 to R.drawable.citroenec4, 8 to R.drawable.citroeneberlingo,
        9 to R.drawable.citroenespacetourer, 10 to R.drawable.cupraborn,
        11 to R.drawable.daciaspring, 12 to R.drawable.ds3,
        13 to R.drawable.fiat500, 14 to R.drawable.fordmustangmache,
        15 to R.drawable.hondae, 16 to R.drawable.hyundaiioniq,
        17 to R.drawable.hyundaiioniq5, 18 to R.drawable.hyundaikauai,
        19 to R.drawable.jaguaripace, 20 to R.drawable.kiaev6,
        21 to R.drawable.kianiro2, 22 to R.drawable.lexusux,
        23 to R.drawable.mazdamx30, 24 to R.drawable.mercedeseqa,
        25 to R.drawable.mercedeseqc, 26 to R.drawable.mercedeseqs,
        27 to R.drawable.mercedeseqv, 28 to R.drawable.mini,
        29 to R.drawable.nissanleaf40, 30 to R.drawable.nissanleaf62,
        31 to R.drawable.opelcorsa, 32 to R.drawable.opelmokka,
        33 to R.drawable.opelcombo, 34 to R.drawable.peugeot208,
        35 to R.drawable.peugeot2008, 36 to R.drawable.peugeotrifter,
        37 to R.drawable.peugeottraveller, 38 to R.drawable.porschetaycan,
        39 to R.drawable.renaultzoe110, 40 to R.drawable.renaultzoe135,
        41 to R.drawable.renaulttwingo, 42 to R.drawable.skodaenyaq60,
        43 to R.drawable.skodaenyaq80, 44 to R.drawable.skodaenyaq80x,
        45 to R.drawable.smartfortwo, 46 to R.drawable.tesla3,
        47 to R.drawable.teslas, 48 to R.drawable.teslax,
        49 to R.drawable.toyotabz4x, 50 to R.drawable.vwid3,
        51 to R.drawable.vwid4
    )
}